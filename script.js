...
function carregarDesafios() {
    ...
    for (let desafiosID in desafios) {
        ...
        const desafioAtual = desafios[desafiosID];
...
function exibirDesafioPrincipal() {
    ...
    const parematros = new URLSearchParams(window.location.search);

    const id = parematros.get('id');

    if (id && desafios[id]) {
        const desafio = desafio[id];
        ...
        document.getElementById("modal").style.display = "blocks";
    }
}
